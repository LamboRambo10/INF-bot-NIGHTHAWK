...
async def event_scheduler():
    ...
    if minute == END_MINUTE and event_active:
        ...
        if len(current_participants) == 0:
            await channel.send("😢 **No one joined the Ammo Car event. Better luck next hour!**")
        else:
...
async def force_start(ctx):
    ...
    current_event_message = msg
    await ctx.send(f"🚨 **Inf lista je pocela imate do :{str(END_MINUTE).zfill(2)} da udete i pobjednik vozi ammo!**")
